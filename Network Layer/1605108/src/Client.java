...
public class Client {
    public static void main(String[] args) throws InterruptedException {
        ...
        */
        ...
        ArrayList<EndDevice> activeClients = (ArrayList<EndDevice>) networkUtility.read();
       // System.out.println(endDevice.getDeviceID() + " , " + endDevice.getIpAddress());
        for (int i = 0; i <100 ; i++) {
            String s = new String("randomMSG");
            Random random = new Random();
            int activeClientID = random.nextInt(activeClients.size());
            EndDevice receiver = activeClients.get(activeClientID);
          //  System.out.println(i + " : " + receiver.getIpAddress());
            if(i == 20)
            {
                Packet packet = new Packet(s,"SHOW_ROUTE",endDevice.getIpAddress(),receiver.getIpAddress());
            ...
            {
                Packet packet = new Packet(s,"",endDevice.getIpAddress(),receiver.getIpAddress());
                networkUtility.write(packet);
                System.out.println(networkUtility.read());
                //System.out.println(networkUtility.read());
            }
