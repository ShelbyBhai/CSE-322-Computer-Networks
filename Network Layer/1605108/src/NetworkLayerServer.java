...
public class NetworkLayerServer {
    ...
    public static void main(String[] args) {
        ...
        for (Router r : routers) System.out.println(r.strRoutingTable());
        simpleDVR(1);
        //DVR(1);
        //routers.get(0).updateRoutingTable(routers.get(1));
        for (Router r : routers) System.out.println(r.strRoutingTable());
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        for (int k = 0; k < routers.size(); k++) {
            ...
            for (int i = 0; i < routers.size(); i++) {
                ...
                if(router.getState()) {
                    for (int j = 0; j < router.getNeighborRouterIDs().size(); j++) {
                        int neighbourId = router.getNeighborRouterIDs().get(j);
                        Router neighbourRouter = routerMap.get(neighbourId);
                        if (neighbourRouter.getState()) {
                            boolean currentConvergence = neighbourRouter.sfupdateRoutingTable(router);
                            convergence = convergence | currentConvergence;
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < routers.size(); i++) {
                ...
                if(router.getState()) {
                    for (int j = 0; j < router.getNeighborRouterIDs().size(); j++) {
                        ...
                        Router neighbourRouter = routerMap.get(neighbourId);
                        if (neighbourRouter.getState()) {
                            boolean currentConvergence = neighbourRouter.updateRoutingTable(router);
                            convergence = convergence | currentConvergence;
                        }
                    }
                }
